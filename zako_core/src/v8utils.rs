...
use v8::{Global, HandleScope, Isolate, Local, OwnedIsolate, PinnedRef, Platform, Value};
...
fn read_state(
    handle_scope: &PinnedRef<HandleScope<()>>,
    promise_global: v8::Global<v8::Promise>) -> v8::PromiseState {
    // 获取锁，进入 V8
    // 不需要 Context，Promise 状态检查与 Context 无关，
    // 但 perform_microtask_checkpoint 依赖 Isolate
    let promise = v8::Local::new(&handle_scope, &promise_global);
    promise.state()
}

pub fn check_try_catch<'s>(mut scope: &mut PinnedRef<v8::TryCatch<'s, '_, HandleScope>>,
                   module_specifier: Option<&ModuleSpecifier>) -> Result<ExecutionResult, V8Error> {
    if scope.has_caught() {
        if scope.can_continue(){
            return Ok(ExecutionResult::Exception(
                Global::new(scope,scope.exception().unwrap())));
        }
    ...
    loop {
        ...
        match promise.state() {
            ...
            v8::PromiseState::Pending => {
                ...
                let platform_pumped = Platform::pump_message_loop(&v8::V8::get_current_platform(),
                                           &mut **scope,
                                           true);
                ...
                if !platform_pumped && promise.state() == v8::PromiseState::Pending {
                    return Err(V8Error{
                        message: "Event loop terminated but promise is still pending (Deadlock)".into(),
                            ..Default::default()
                    });
                }
            }
...
where
    F: FnOnce(&mut PinnedRef<v8::TryCatch<'_,'_, HandleScope>>, v8::Local<'_, v8::Context>) -> Option<R>,
{
        ...
        match err{
            ...
            Ok(value)=>{
                match value{
                    Ok(()) => Ok(Ok(result)),
                    Err(exception) => {
                        Ok(Err(Global::new(&mut try_catch,exception)))
                    }
                }
            }
        }
